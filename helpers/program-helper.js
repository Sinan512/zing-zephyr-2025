...
module.exports = {
  setFestName: async (festname, callback) => {
    ...
    if (festTitle) {
      await db
        .collection(collections.FEST_NAME)
        .updateOne(
          { festName: festTitle[0].festName },
          { $set: { festName: festname.festName } }
        )
        .then((data) => {});
    } else {
      await db
  ...
  addTeams: async (teamData) => {
    var db = await connectDB();
    var teamName = await db
      .collection(collections.TEAM_DATA)
      .findOne({ teamName: teamData.teamName });
    if (teamName) {
  ...
  getAllMemberPointsByZone: async () => {
    var db = await connectDB();
    const docs = await db
      .collection(collections.INDIVIDUAL_POINTS)
      .find()
      .toArray();
...
for (const result of allResults) {
    ...
    if (!prog || prog.type !== "Individual") continue;

    let zone = prog.zone;

    // ⭐ If General → determine student's real zone
    if (zone === "General" && result.results) {
        for (const r of result.results) {
            const teamDoc = await db.collection(collections.TEAM_DATA)
                .findOne({ teamName: r.team });

            if (teamDoc) {
                for (const z of ["Pre Zone", "Mid Zone", "High Zone"]) {
                    if (Array.isArray(teamDoc[z]) &&
                        teamDoc[z].some(m => m.member === r.participant)) {
                        zone = z;
                        break;
                    }
                }
    ...
    // If zone still invalid → skip
    if (!["Pre Zone", "Mid Zone", "High Zone"].includes(zone)) continue;
    ...
    const finalZoneData = {
      "Pre Zone": Object.values(agg["Pre Zone"]).sort(
        (a, b) => b.points - a.points
      ),
      "Mid Zone": Object.values(agg["Mid Zone"]).sort(
        (a, b) => b.points - a.points
      ),
      "High Zone": Object.values(agg["High Zone"]).sort(
        (a, b) => b.points - a.points
      ),
      preZone: [],
      midZone: [],
